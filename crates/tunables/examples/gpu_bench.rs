...
fn main() {
    let _ = (DRAG_GAIN, FRICTION, HOME_ZOOM, GRAVITY, SOFTENING, WORKGROUP, GALAXY_DEFAULT_N, RAMP_TARGET_FPS);
    write_markdown("TUNABLES.md").expect("write TUNABLES.md");
    eprintln!("[tunables] wrote TUNABLES.md ({} registered)", guardrails_tunables::all().len());
}
